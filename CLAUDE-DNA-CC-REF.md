...
| v3.0 (CORE) | 2026-07-21 | Interdiction explicite de l'outil `AskUserQuestion` — toujours la convention Q/R texte. Règle « voix de Guillaume » corrigée : priorité au profil projet, garde-fou anti-stub-vide-silencieux, modèle Sonnet par défaut. Suite à la recherche `grech` lettre de motivation. REF non touché. |
| v2.9 (CORE) | 2026-07-05 | Nouveau trigger `grech` : recherche approfondie multi-agents, skill `.claude/skills/grech` — même patron « CORE pointe, skill contient » que `gpose`/`gauto`. REF non touché. |
| v2.8 (CORE) | 2026-07-01 | Patron « CORE pointe, skill contient » généralisé : `gpose` et `gauto`/`gstop` déportés en skills `.claude/skills/gpose` et `.claude/skills/gauto` ; le CORE ne garde que le trigger + les safety interdits non déportables. REF non touché. |
| v2.7 (CORE) | 2026-06-23 | Ajout module Asana « nous 3 » (doctrine séparée `CLAUDE-DNA-ASANA.md`) + règle voix de Guillaume + convention remontées `to-os/`. REF non touché. |
| v2.6 (CORE) | 2026-06-13 | Ajout règle voix de Guillaume + convention remontées `to-os/` (première version, complétée par v2.7). REF non touché. |
...
| v2.4 (CORE) | 2026-06-13 | Ajout de la convention `TODO.md` projet (backlog persistant), après l'inbox `q:`. Routine de démarrage lit `REPRISE.md` puis `TODO.md`. REF non touché. |
| v2.3 (CORE) | 2026-05-27 | Ajout de la convention d'inbox de questions différées `q:` — fichier hybride projet + global (`INBOX-QUESTIONS.md`). REF non touché. |
| v2.2 (CORE) | 2026-05-27 | Ajout de la règle `git pull` obligatoire au démarrage + hook SessionStart v2.1 avec `git pull --rebase --autostash origin main` automatique (échec doux hors ligne). REF non touché. |
| v2.1 (CORE) | 2026-05-22 | Combo autonome — trigger `gauto` / arrêt `gstop` : boucle pilotée (ne jamais s'arrêter spontanément), persistance & git par étape, économie API, 3 conditions d'arrêt, safety interdits (pas de merge/force-push/`--no-verify`/modif hooks). REF non touché. |
