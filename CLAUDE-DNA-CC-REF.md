...
- **`from-chat/` vide** : ne rien dire, passer à l'étape suivante.
- **`from-chat/` contient `.DS_Store` ou README** : les ignorer, ne traiter que les `.md` hors README.
- **`from-cc/_upload-status.json` absent** : signaler une seule fois à Guillaume + proposer création depuis template.
...
.claude/                   ← config CC projet (commitée)
  ...
  hooks/session-start.sh         (charge DNA-CC-CORE + check from-cc, marche cloud + local)
...
## Migration projet legacy (v≤1.4 / v1.5-1.9 → v2.0)

Quand CC ouvre un projet créé sous DNA ≤ v1.9 et que Guillaume veut le mettre à jour (mot-clé `migrate-projet`, ou "sync DNA" qui est l'ancien terme), suivre cette procédure. **Toujours `gpose` d'abord** : reformuler le plan, chiffrer le coût, demander confirmation avant d'exécuter.
...
| `knowledge/CONNAISSANCE-PROJET.md` (legacy monolithique) | Ancien Project Knowledge unique | À découper en `from-cc/knowledge-<sujet>-vX.Y.md` OU garder en l'état et renommer en `from-cc/knowledge-projet-v1.0.md` (migration minimale). Décision à prendre avec Guillaume. |
...
3. **Attendre choix**.
4. **Exécuter** dans cet ordre : (a) update `CLAUDE.md` avec bloc fallback CORE, (b) supprimer copies DNA legacy, (c) créer `from-cc/` depuis templates, (d) migrer knowledge legacy selon choix, (e) initialiser `_upload-status.json` / `_TODO.md` / `_track-log.md`, (f) installer `.claude/hooks/session-start.sh` v2.0 (DNA_URL → CORE), (g) commit "migrate: project to DNA v2.0", (h) lister les uploads pending pour Guillaume.
